...
# Function to calculate MACD and normalize it
    ...
    fig_rrg.add_shape(type="rect", x0=100, y0=100, x1=1*(relative_strength.min().min()), y1=1*(momentum.min().min()),
                      xref="x", yref="y",
                      fillcolor="lightcoral", opacity=0.3, layer="below", line_width=0)
    
    fig_rrg.add_shape(type="rect", x0=100, y0=100, x1=1*(relative_strength.min().min()), y1=1*(momentum.max().max()),
                      xref="x", yref="y",
